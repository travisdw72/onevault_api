-- =============================================
-- Comprehensive OneVault API Role - FIXED VERSION
...
BEGIN
    ...
    LOOP
        IF EXISTS (SELECT 1 FROM information_schema.schemata WHERE schema_name = schema_name) THEN
            EXECUTE format('GRANT USAGE ON SCHEMA %I TO onevault_api_full', schema_name);
            EXECUTE format('GRANT ALL ON ALL TABLES IN SCHEMA %I TO onevault_api_full', schema_name);
            EXECUTE format('GRANT ALL ON ALL SEQUENCES IN SCHEMA %I TO onevault_api_full', schema_name);
            EXECUTE format('GRANT ALL ON ALL FUNCTIONS IN SCHEMA %I TO onevault_api_full', schema_name);
            EXECUTE format('ALTER DEFAULT PRIVILEGES IN SCHEMA %I GRANT ALL ON TABLES TO onevault_api_full', schema_name);
            EXECUTE format('ALTER DEFAULT PRIVILEGES IN SCHEMA %I GRANT ALL ON SEQUENCES TO onevault_api_full', schema_name);
            EXECUTE format('ALTER DEFAULT PRIVILEGES IN SCHEMA %I GRANT ALL ON FUNCTIONS TO onevault_api_full', schema_name);
            RAISE NOTICE '✅ % schema permissions granted', schema_name;
        ELSE
            RAISE NOTICE '⚠️  % schema not found - skipping', schema_name;
        END IF;
    END LOOP;
END $$;
...
BEGIN
    ...
    -- Count granted permissions - FIXED column names
