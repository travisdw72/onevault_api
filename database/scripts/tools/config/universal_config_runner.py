...
Demonstrates single source of truth principle with multiple config format support
"""
...
class UniversalConfigRunner:
    """
    Configuration runner that supports multiple formats:
    - JSON (.json)
    - YAML (.yaml, .yml) 
    - TOML (.toml)
    - Python (.py)
    ...
    def execute_query(self, query_name: str, params: Optional[tuple] = None) -> List[Dict[str, Any]]:
        ...
        if isinstance(query, dict):
            # Query might be an object with SQL and metadata
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python universal_config_runner.py <config_file> [query_name]")
        print("\nSupported formats:")
        print("  - JSON (.json)")
        print("  - YAML (.yaml, .yml)")
        print("  - TOML (.toml)")
        print("  - Python (.py)")
        print("\nExamples:")
        print("  python universal_config_runner.py config.yaml")
        print("  python universal_config_runner.py config.py get_user_stats")
        return 1
    
    config_file = sys.argv[1]
    query_name = sys.argv[2] if len(sys.argv) > 2 else None
    
    runner = UniversalConfigRunner(config_file)
