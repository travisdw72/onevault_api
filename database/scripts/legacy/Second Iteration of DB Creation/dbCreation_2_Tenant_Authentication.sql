...
DO $$ BEGIN
    RAISE NOTICE 'Starting Tenant Authentication setup...';
    -- Add rollback point
    SAVEPOINT tenant_authentication_start;
EXCEPTION WHEN OTHERS THEN
    -- Roll back to start if anything fails
    ROLLBACK TO tenant_authentication_start;
    RAISE EXCEPTION 'Tenant Authentication setup failed: %', SQLERRM;
END $$;
...
CREATE TABLE auth.tenant_profile_s (
    ...
    created_date TIMESTAMP WITH TIME ZONE DEFAULT util.current_load_date(),  -- ADD THIS
    last_updated_date TIMESTAMP WITH TIME ZONE DEFAULT util.current_load_date(),  -- ADD THIS
    record_source VARCHAR(100) NOT NULL,
