...
DO $$ BEGIN
    ...
    SAVEPOINT cleanup_start;
EXCEPTION WHEN OTHERS THEN
    ROLLBACK TO cleanup_start;
    RAISE EXCEPTION 'Cleanup failed: %', SQLERRM;
END $$;

-- Drop functions and procedures in dependency order
DROP FUNCTION IF EXISTS staging.validate_user_request(BYTEA) CASCADE;
DROP FUNCTION IF EXISTS staging.process_user_creation(BYTEA) CASCADE;
DROP FUNCTION IF EXISTS raw.handle_user_request(JSONB) CASCADE;

-- Drop triggers
DROP TRIGGER IF EXISTS tr_user_request_validation ON raw.user_request_details_s CASCADE;
DROP TRIGGER IF EXISTS tr_user_creation_processing ON staging.user_creation_s CASCADE;
...
DROP TABLE IF EXISTS staging.user_creation_validation_s CASCADE;
DROP TABLE IF EXISTS staging.user_creation_s CASCADE;
DROP TABLE IF EXISTS staging.user_creation_h CASCADE;

-- Create savepoint after successful cleanup
