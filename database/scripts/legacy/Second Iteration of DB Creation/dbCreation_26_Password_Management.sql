-- =============================================
-- dbCreation_26_Password_Management.sql
...
BEGIN
    ...
    IF v_user_auth.password_hash IS NOT NULL THEN
        v_credentials_valid := (crypt(v_current_password, v_user_auth.password_hash::text) = v_user_auth.password_hash::text);
    ELSE
    ...
    -- FIXED: End-date the previous record FIRST to avoid unique constraint violation
    UPDATE auth.user_auth_s
    SET load_end_date = v_load_date
    WHERE user_hk = v_user_hk
    AND load_end_date IS NULL;
    
    -- THEN: Create new auth satellite record with updated password
...
BEGIN
    ...
    -- THEN: Create new auth satellite record with reset password
    INSERT INTO auth.user_auth_s (
        user_hk,
        load_date,
        hash_diff,
    ...
    -- Create audit record for password reset
...
BEGIN
    ...
    -- THEN: Create new auth satellite record
    INSERT INTO auth.user_auth_s (
        user_hk,
        load_date,
        hash_diff,
