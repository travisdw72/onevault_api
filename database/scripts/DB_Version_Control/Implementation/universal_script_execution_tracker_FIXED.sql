-- =============================================================================
-- Universal Script Execution Tracker - FIXED VERSION
...
BEGIN
    ...
    INSERT INTO script_tracking.script_execution_s VALUES (
        ...
        CURRENT_TIMESTAMP,
        SESSION_USER,
        current_user,                            -- FIXED: SESSION_USER function is OK, the column name was the issue
        ...
        'SCRIPT_TRACKER'
            ...
            -- Log to existing audit system if available (FIX 1: Explicit type casting)
        BEGIN
...
BEGIN
    ...
    -- Log completion to existing audit system (FIX 1: Explicit type casting)
    BEGIN
        IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'log_audit_event') THEN
            PERFORM util.log_audit_event(
                'SCRIPT_EXECUTION'::text,            -- p_event_type (explicit cast)
