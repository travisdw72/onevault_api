-- =====================================================
-- URGENT FIX: Correct Audit Function Parameter Order
-- =====================================================
-- Issue: AI observation function calling audit with wrong parameter order
-- Fix: Correct the parameter order to match 8-parameter function signature
-- =====================================================
...
DECLARE
    ...
    -- Variables now in main scope (scope bug fixed)
    v_entity_hk BYTEA;
    v_sensor_hk BYTEA;
...
BEGIN
    ...
    -- *** FIXED: Correct audit function parameter order ***
    -- 8-parameter function: (event_type, event_severity, description, source_ip, user_agent, user_hk, threat_level, metadata)
    PERFORM audit.log_security_event(
        'AI_OBSERVATION_LOGGED',                    -- p_event_type
        CASE v_severity_level                       -- p_event_severity
            WHEN 'emergency' THEN 'CRITICAL'
            WHEN 'critical' THEN 'HIGH'
            ELSE 'MEDIUM'
        END,
        'AI observation logged: ' || v_observation_type || ' (' || v_severity_level || ')',  -- p_event_description
        v_ip_address,                               -- p_source_ip_address (was in wrong position!)
        v_user_agent,                               -- p_user_agent  
        NULL,                                       -- p_affected_user_hk (no specific user for AI observations)
        'MEDIUM',                                   -- p_threat_level
        jsonb_build_object(                         -- p_event_metadata
...
-- WHAT THIS FIX DOES:
-- =====================================================
-- BEFORE: audit.log_security_event(event_type, severity, description, NULL, user_agent, ip_address, threat_level, metadata)
-- AFTER:  audit.log_security_event(event_type, severity, description, ip_address, user_agent, NULL, threat_level, metadata)
-- 
-- The 8-parameter function expects: (event_type, severity, description, ip_address, user_agent, user_hk, threat_level, metadata)
-- We were passing ip_address in position 6 instead of position 4!
--
-- RESULT: Audit logging now works with proper parameter order! 🚀
-- ===================================================== 